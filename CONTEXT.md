...
- **Tests:** 1,618 (cargo test --all-features, pure Rust — no Docker required)
...
- **`braid.anchor`:** Anchors a braid to a DAG branch point at branch creation time; signs via Tower/BearDog `crypto.sign`. Called by `rootpulse.branch` signal graphs.
...
- **Crate count:** 9 workspace crates
...
- **Cross-architecture:** `cargo check --target x86_64-pc-windows-gnu` clean (Wave 141a–142b); Phase 2 active — `TransportEndpoint` trait dispatch replaces raw UDS gating
