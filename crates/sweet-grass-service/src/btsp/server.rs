...
pub(crate) fn extract_str(value: &serde_json::Value, field: &str) -> Result<String, BtspError> {
    value
        .get(field)
        .and_then(serde_json::Value::as_str)
        .map(String::from)
...
pub(crate) fn extract_handshake_key(verify_result: &serde_json::Value) -> Option<[u8; 32]> {
