...
// ── braid.verify behavioral tests (P2) ──────────────────────────────────
...
async fn test_braid_verify_unsigned_braid() {
    ...
    assert!(checks[0]["signing_hash"].as_str().unwrap().starts_with("sha256:"));
...
async fn test_braid_verify_crypto_down_permissive() {
    ...
    assert!(result.is_ok(), "braid.verify must not error when crypto is unavailable");
    let val = result.unwrap();
...
async fn test_braid_verify_returns_attribution_metadata() {
    ...
    assert!(result["attributed_to"].as_str().unwrap().starts_with("did:"));
